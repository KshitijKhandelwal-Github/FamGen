...
def CreateTree(filename):
    ...
    df = df[['ID', 'Name', 'Sex', 'DoB', 'DoD', 'Place of birth', 'Job', 'ParentID','SpouseID']]
    ...
    for index, row in df.iterrows():
        ...
        node_color = 'lightpink' if row['Sex'] == 'F' else 'lightblue' if row['Sex'] == 'M' else 'lightgray'
        ...
        if str(row['SpouseID']) != '':
            ...
            if spouse_id not in spouse_clusters:
                ...
                spouse_clusters[spouse_id].attr(label='Couple', color='yellow', style='filled')
